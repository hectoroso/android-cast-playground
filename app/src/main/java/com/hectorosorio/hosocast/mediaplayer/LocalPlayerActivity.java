package com.hectorosorio.hosocast.mediaplayer;
...
//import com.hectorosorio.hosocast.queue.ui.QueueListViewActivity;
//import com.hectorosorio.hosocast.settings.CastPreference;
...
public class LocalPlayerActivity extends AppCompatActivity {
    ...
    private void setupCastListener() {
        mCastConsumer = new VideoCastConsumerImpl() {
            ...
            public void onApplicationDisconnected(int errorCode) {
                Log.d(TAG, "onApplicationDisconnected() with errorCode: " + errorCode);
                updatePlaybackLocation(PlaybackLocation.LOCAL);
            }
    ...
    private void play(int position) {
        Log.d(TAG, "play pos=" + position);
        startControllersTimer();
        switch (mLocation) {
            case LOCAL:
                Log.d(TAG, "play LOCAL pos=" + position);
                mVideoView.seekTo(position);
                mVideoView.start();
                break;
            case REMOTE:
                Log.d(TAG, "play REMOTE pos=" + position);
                mPlaybackState = PlaybackState.BUFFERING;
                updatePlayButton(mPlaybackState);
                try {
    ...
    private void togglePlayback() {
        ...
        switch (mPlaybackState) {
            case PAUSED:
                switch (mLocation) {
                    case LOCAL:
                        Log.d(TAG, "togglePlayback PAUSED Local");
                        mVideoView.start();
                        ...
                        break;
                    case REMOTE:
                        Log.d(TAG, "togglePlayback PAUSED Remote");
                        try {
            ...
            case IDLE:
                switch (mLocation) {
                    case LOCAL:
                        Log.d(TAG, "togglePlayback IDLE Local");
                        mVideoView.setVideoURI(Uri.parse(mSelectedMedia.getContentId()));
                        mVideoView.seekTo(0);
                        mVideoView.start();
                        mPlaybackState = PlaybackState.PLAYING;
    ...
    private void updateMetadata(boolean visible) {
        ...
        if (!visible) {
            ...
            mDescriptionView.setText(mSelectedMedia.getCustomData().optString("description")); //VideoProvider.KEY_DESCRIPTION
